...
export default function MapViewContent(props: MapViewContentProps) {
  ...
  const handleLegendExpand = (e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    setLegendCollapsed(false)
  }

  const handleLegendCollapse = (e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
              ...
              <div className="max-h-[min(65vh,380px)] space-y-2.5 overflow-y-auto p-3">
                ...
                <div className="border-t border-border/40 pt-2">
                  <button
                    type="button"
                    onClick={handleLegendCollapse}
                    className="flex w-full items-center justify-between rounded-lg border border-border/50 bg-secondary/30 px-2.5 py-2 text-[11px] font-medium text-muted-foreground transition-colors hover:bg-secondary/45 hover:text-foreground"
                    title="Hide legend"
                  >
