...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    ...
    <Placemark>
      <name>Start</name>
      <Point>
        <coordinates>${start.lng},${start.lat},${start.elevation ?? 0}</coordinates>
      </Point>
    </Placemark>
    <Placemark>
      <name>End</name>
      <Point>
        <coordinates>${end.lng},${end.lat},${end.elevation ?? 0}</coordinates>
      </Point>
    </Placemark>
    <Placemark>
      <name>${safeRouteName} path</name>
      <LineString>
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `${route.name.replace(/\s+/g, '-').toLowerCase()}.kml`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
